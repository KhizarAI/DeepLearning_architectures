...
class UNet(nn.Module):
    ...
    def forward(self, image):
        #Encode
        x1 = self.down_conv1(image) # for concatenation
        x2 = self.max_pool(x1)
        x3 = self.down_conv2(x2)    # for concatenation
        x4 = self.max_pool(x3)
        x5 = self.down_conv3(x4)    # for concatenation
        x6 = self.max_pool(x5)
        x7 = self.down_conv4(x6)    # for concatenation
        x8 = self.max_pool(x7)
